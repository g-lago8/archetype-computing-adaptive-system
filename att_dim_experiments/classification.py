...
def get_data(args):
    ...
    return (train_dataset, train_target.astype(int)), (val_dataset, val_target.astype(int)), (test_dataset, test_target.astype(int))
...
def evaluate(states:torch.Tensor, labels:torch.Tensor, readout:RidgeClassifier):
    n_modules, n_hid = states.shape[1], states.shape[-1]
    X = states[-1, :, :, 0, :].reshape(states.shape[2], n_modules * n_hid).numpy()  # (len_seq, n_mod, n_samples, 2, h_dim) -> (n_samples, n_mod * h_dim)
    y = labels.numpy()
...
def run_classification():
    ...
    wandb.init(project=f"archetype-classification", config=args_dict)
